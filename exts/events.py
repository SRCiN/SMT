    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, commands.CommandNotFound):
            ...
            if isinstance(error, commands.MemberNotFound) or isinstance(
                ...
                await ctx.send(
                    f":mag_right: {ctx.author.mention} - `{error.argument}`(이)랑 관련된 채널을 못 찾았어. 확인하고 다시 해줄래?"
                )
            elif isinstance(error, commands.ChannelNotReadable):
                await ctx.send(
                    f"<:cs_no:659355468816187405> {ctx.author.mention} - `{error.argument}`(이)랑 관련된 채널은 내가 접근할 수 없는 것 같아."
                )
            elif isinstance(error, commands.RoleNotFound):
                await ctx.send(
                    f":mag_right: {ctx.author.mention} - `{error.argument}`(이)랑 관련된 역할을 못 찾았어. 확인하고 다시 해줄래?"
                )
            else:
                await ctx.send(
            ...
            await ctx.send(
                f"<:cs_no:659355468816187405> {ctx.author.mention} - 미안해, 그 명령어는 아무나 사용할 수 없도록 제한되어 있어."
            )
            ...
            await ctx.send(
                f":stopwatch: {ctx.author.mention} - 그건 좀 더 이따가 사용해줄래? {round(error.retry_after, 2)}초 정도 뒤면 될 거 같아."
            )
        else:
