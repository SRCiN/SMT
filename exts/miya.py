            ...
            for word in filter:
                if word in msg.content and msg.channel.id != 663806592277545005:
                    ...
                    await msg.guild.ban(
                        ...
                        reason="서버 활동 기간이 5일 미만이고, 부적절한 언행을 사용했습니다.",
                    )
    ...
    async def on_member_join(self, member):
        ...
        if member.guild.id == 564418977627897887:
            ...
            if delay <= limit:
                try:
                    await member.send(
                        f"<:cs_stop:665173353874587678> {member.mention} - 지원 서버 정책에 따라, 생성된 지 3일이 되지 않은 계정은 추방됩니다."
                    )
                except:
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if (
            ...
            if ch.name.startswith("스레드_"):
                ...
                name = ch.name.replace("스레드", "종료됨")
                await ch.edit(name=name)
                try:
                    await member.send(
                        f"<:cs_leave:659355468803866624> {member.mention} - 스레드가 종료되었어요. 활성화하려면 `@Project. SMT#2979 티켓 {ch.id}` 명령어를 사용해주세요."
                    )
                except:
            ...
            if (
                ...
                async for msg in channel.history(limit=500):
                    if msg.author == self.SMT.user:
                        message = msg
                await message.reply(
                    "<:cs_yes:659355468715786262> @here - 스레드가 티켓 관리자에 의해 다시 활성화됐습니다."
                )
