...
class NoName(commands.Cog, name="Management"):
    ...
    async def health_check(self):
        ...
        if "07시 30분" in kortime:
            ...
            for row in rows:
                ...
                hcs = await hcskr.asyncSelfCheck(name=row[1], birth=row[2], level=row[3], area=row[4], schoolname=row[5], customloginname=row[6], password=row[7])
                if hcs['code'] == "SUCCESS":
                    success += 1
                    if row[8] == "true":
                ...
                if not rows:
                    ...
                    while i < 7:
                        ...
                        try:
                            ...
                            await msg.delete()
                            await queue.delete()
                            information.append(str(msg.content))
                            i += 1
                    asdf = await ctx.send(f"<:cs_id:659355469034422282> {ctx.author.mention} - 잠시만, 확인 좀 해보고.\n자가진단 테스트 : <a:cs_wait:659355470418411521> 진행 중")
                    try:
                        hcs = await hcskr.asyncSelfCheck(name=information[0], birth=information[1], level=information[2], area=information[3], schoolname=information[4], customloginname=information[5], password=information[6])
                        ...
                        if hcs['code'] != "SUCCESS":
                            ...
                            await asyncio.sleep(1)
                            try:
                                await c.execute(f"INSERT INTO health(user_id, name, birth, level, area, school, custom, password, notify) VALUES('{ctx.author.id}', '{information[0]}', '{information[1]}', '{information[2]}', '{information[3]}', '{information[4]}', '{information[5]}', '{information[6]}', 'true')")
                                await o.commit()
                            except Exception as e:
                                debug = self.SMT.get_channel(783621627875164230)
                                await debug.send(f"도와주고 있는데, 문제가 좀 생긴 것 같아. 네가 확인해줄래? ```{e}```")
