        ...
        if todo == "추가":
                ...
                if not rows:
                    ...
                    try:
                        ...
                        if str(reaction.emoji
                            ...
                            await o.commit()
                            await ctx.send(
                                f"➕ {ctx.author.mention} - 알았어! `{name}` 단어를 기억해둘게."
                            )
                        else:
                            await msg.delete()
                else:
                    await ctx.send(
                        f"<:cs_no:659355468816187405> {ctx.author.mention} - 그건 이미 있는 거 같은데, 수정하고 싶으면 수정해도 되고."
                    )
            else:
                await ctx.send(
                    ...
                    if rows[0][3] != "true":
                        ...
                        try:
                            ...
                            if str(reaction.emoji
                                ...
                                await c.execute(
                                    f"UPDATE words SET teachedBy = '{ctx.author.id}' WHERE name = '{name}'"
                                )
                                await o.commit()
                                await ctx.send(
                                    f"<:cs_reboot:659355468791283723> {ctx.author.mention} - 알았어! `{name}` 단어를 다시 기억해둘게."
                                )
                            else:
                                await msg.delete()
                    else:
                        await ctx.send(
                            f":lock: {ctx.author.mention} - 어, 그 단어는 잠금 설정되어 있어. 다른 걸 수정해볼래?"
                        )
                else:
                    await ctx.send(
                        f"<:cs_no:659355468816187405> {ctx.author.mention} - 네가 말해준 단어를 찾지 못했어. 다른 걸로 해볼래?"
                    )
            else:
                await ctx.send(
                    f"<:cs_no:659355468816187405> {ctx.author.mention} - 어, 그게 아니었던 것 같은데. 다시 생각해보는 건 어때?"
            ...
            if rows:
                if rows[0][3] != "true":
                    embed = discord.Embed(
                        title="잠깐만, 그거 정말 맞아?",
                        description=f"`{name}`(이)라고 물어봐도 모른 척 하면 되는거야?",
                        color=0xBE1010,
                        timestamp=datetime.datetime.utcnow(),
                    )
                    embed.set_thumbnail(url=self.SMT.user.avatar_url_as(
                    ...
                    await ctx.send(
                        f":lock: {ctx.author.mention} - 어, 그 단어는 잠금 설정되어 있어. 다른 걸 삭제해볼래?"
                    )
            else:
                await ctx.send(
                    f"<:cs_no:659355468816187405> {ctx.author.mention} - 네가 말해준 단어를 찾지 못했어. 다른 걸로 해볼래?"
            ...
            if staff in ctx.author.roles:
                if rows:
                    if rows[0][3] != "true":
                        embed = discord.Embed(
                            title="잠깐만, 그거 정말 맞아?",
                            description=f"`{name}` 단어를 아무도 바꾸지 못하게 할건데, 괜찮아?",
                            color=0xBE1010,
                            timestamp=datetime.datetime.utcnow(),
                        )
                        embed.set_thumbnail(url=self.SMT.user.avatar_url_as(
                        ...
                        try:
                            ...
                            if str(reaction.emoji
                                ...
                                await o.commit()
                                await ctx.send(
                                    f":lock: {ctx.author.mention} - 그래, 뭐. 괜찮다면야. `{name}` 단어를 제한해둘게."
                                )
                            else:
                                await msg.delete()
                    else:
                        await ctx.send(
                            f":lock: {ctx.author.mention} - 이미 잠겨 있는 단어라서 의미 없어 보이는데."
                        )
                else:
                    await ctx.send(
                        f"<:cs_no:659355468816187405> {ctx.author.mention} - 네가 말해준 단어를 찾지 못했어. 다른 걸로 해볼래?"
                ...
                await ctx.send(
                    f"<:cs_no:659355468816187405> {ctx.author.mention} - 그건 어떤 역할을 가지고 있는 사람만 사용할 수 있어. 다음에 다시 해줄래?"
                )
        elif todo == "해제":
            staff = ctx.guild.get_role(719061173567488010)
            if staff in ctx.author.roles:
                    ...
                    if rows[0][3] != "false":
                        ...
                        try:
                            ...
                            if str(reaction.emoji
                                ...
                                await o.commit()
                                await ctx.send(
                                    f":unlock: {ctx.author.mention} - 그래, 뭐. `{name}` 단어의 제한을 풀어 놓을게."
                                )
                            else:
                                await msg.delete()
                    else:
                        await ctx.send(
                            f":unlock: {ctx.author.mention} - 그거 딱히 내가 제한 걸어놓은 적 없던 거 같은데, 상관 없으려나."
                        )
                else:
                    await ctx.send(
                        f"<:cs_no:659355468816187405> {ctx.author.mention} - 네가 말해준 단어를 찾지 못했어. 다른 걸로 해볼래?"
